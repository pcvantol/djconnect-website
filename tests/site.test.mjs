...
test("homepage promotes Ask DJ as a major product feature", async () => {
  ...
  assert.match(index, /backend capabilities; Spotify-profieldata wordt alleen gebruikt waar Spotify Direct/);
...
test("how-to-start page covers setup flow", async () => {
  ...
  assert.match(start, /Voer de pairing code lokaal in of scan de QR-code uit Home Assistant/);
  ...
  assert.match(start, /Raspberry Pi gebruikt lokale runtime\/device-connectiviteit en blijft local-only/);
...
test("privacy policy page covers App Store requirements", async () => {
  ...
  assert.match(privacy, /Music Assistant provider credentials, Home Assistant tokens en koppelgegevens blijven/);
  assert.match(privacy, /Remote pairing wordt niet ondersteund/);
  assert.match(privacy, /ESP32, Raspberry Pi en apps bewaren geen Spotify credentials/);
