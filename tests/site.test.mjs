...
test("homepage promotes Ask DJ as a major product feature", async () => {
  ...
  assert.match(index, /pushmeldingen gebruiken als wake- of synchint voor Ask DJ via de DJConnect pushrelay/);
  assert.match(index, /Push blijft beperkt tot Ask DJ-antwoorden of wachtende keuzes/);
...
test("privacy policy page covers App Store requirements", async () => {
  ...
  assert.match(privacy, /Pushberichten zijn alleen wake- of synchints voor Ask DJ-antwoorden of wachtende keuzes/);
  ...
  assert.match(privacy, /Push messages are only wake or sync hints for Ask DJ replies or waiting choices/);
